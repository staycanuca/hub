...
def get_popular_tmdb(page=1):
    return _call_tmdb_api('movie/popular', {'page': page, 'language': 'en-US'})

def get_movies_by_year_tmdb(year, page=1):
    params = {'primary_release_year': year, 'sort_by': 'popularity.desc', 'page': page, 'language': 'en-US'}
    return _call_tmdb_api('discover/movie', params)

def get_genres_tmdb():
    return _call_tmdb_api('genre/movie/list', {'language': 'en-US'})

def get_movies_by_genre_tmdb(genre_id, page=1):
    params = {'with_genres': genre_id, 'sort_by': 'popularity.desc', 'page': page, 'language': 'en-US'}
    return _call_tmdb_api('discover/movie', params)

def search_tmdb(query, page=1):
    return _call_tmdb_api('search/movie', {'query': query, 'page': page, 'language': 'en-US'})

# --- TMDb TV Show Functions ---
def get_popular_tv_tmdb(page=1):
    return _call_tmdb_api('tv/popular', {'page': page, 'language': 'en-US'})

def get_tv_by_year_tmdb(year, page=1):
    params = {'first_air_date_year': year, 'sort_by': 'popularity.desc', 'page': page, 'language': 'en-US'}
    return _call_tmdb_api('discover/tv', params)

def get_tv_genres_tmdb():
    return _call_tmdb_api('genre/tv/list', {'language': 'en-US'})
...
def search_tv_tmdb(query, page=1):
    return _call_tmdb_api('search/tv', {'query': query, 'page': page, 'language': 'en-US'})
